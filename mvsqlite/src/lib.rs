...
use mvfs::vfs::AbstractCore;
use mvsqlite_core::{Core, CoreConfig};

use crate::{util::get_conn, vfs::MultiVersionVfs};
...
fn init_with_options_impl(opts: InitOptions) {
    ...
    let cluster = std::env::var("MVSQLITE_FDB_CLUSTER")
        .unwrap_or_else(|_| String::from("/etc/foundationdb/fdb.cluster"));
    let raw_data_prefix = std::env::var("MVSQLITE_RAW_DATA_PREFIX").unwrap_or_default();
    let metadata_prefix = std::env::var("MVSQLITE_METADATA_PREFIX").unwrap_or_default();
    let auto_create_namespace =
        std::env::var("MVSQLITE_AUTO_CREATE_NAMESPACE").unwrap_or_default() == "1";
    let content_cache_size = std::env::var("MVSQLITE_CONTENT_CACHE_SIZE")
        .ok()
    ...
    let core_config = CoreConfig {
        ...
        content_cache_size,
        auto_create_ns: auto_create_namespace,
    };

    // See mvfs::vfs::AbstractCore's doc comment: in fork-tolerant mode, FDB's
    // network boot and Core::open are deferred to first use in each process,
    // since a forked child cannot safely inherit an already-booted FDB
    // connection from its parent. This only handles fork-before-first-use
    // correctly - forking after this process has already touched FDB is
    // unsupported by FDB's client regardless of how this is structured.
    let core = if opts.fork_tolerant {
        AbstractCore::Lazy(Arc::new(mvfs::vfs::LazyCore::new(core_config)))
    } else {
        mvsqlite_core::boot::boot();
        let core = futures::executor::block_on(Core::open(core_config))
            .expect("failed to open FDB core");
        AbstractCore::Prebuilt(Arc::new(core))
    };
...
pub extern "C" fn init_mvsqlite() {
    ...
    GLOBAL_INIT_DONE.call_once(|| {
        ...
        init_with_options_impl(InitOptions { fork_tolerant });
    });
}
...
unsafe extern "C" fn mv_time2version(
    ...
    let info = conn.inner.time2version(timestamp_secs as u64);
